...
Connection.prototype.insertOrUpdate = function (insert, sql, bindParams, options, callback) {
    ...
    var useExecuteMany = (options && options.useExecuteMany && self.executeMany && typeof self.executeMany === 'function');
    if (useExecuteMany) {
        ...
        var additionalLobInfo;
        for (index = 1; index < bindParams.length; index++) {
            additionalLobInfo = self.modifyParams(sql, bindParams[index], options); //modify bind params for next rows as well
        ...
        if ((!error) && results) {
            if (results.rowsAffected >= 1 && lobColumns && lobColumns.length) {
                ...
                if (useExecuteMany) {
                    ...
                    var tasks = [];
                    var createTask = function (outBinds, lobExecuteData, rowsAffected) {
                        return function handleResults(asyncCallback) {
                            recordWriter.writeMultiple(outBinds, lobExecuteData, function onWriteDone(operationError, operationResults) {
                                ...
                                if (!operationError && operationResults) {
                                    operationResults.rowsAffected = rowsAffected;
                                }
                    ...
                    for (index = 0; index < bindParams.length; index++) {
                        tasks.push(createTask(results.outBinds[index], lobData[index], results.dmlRowCounts[index]));
                    }

                    asyncLib.series(tasks, function onBatchDone(batchError, batchResults) {
                        var batchCallback = self.createModifyCallback(callback, autoCommit, batchResults);
                ...
                var updatedResults = [];
                for (index = 0; index < results.dmlRowCounts.length; index++) {
                    updatedResults.push({
                        outBinds: {},
                        rowsAffected: results.dmlRowCounts[index]
                    });
                }
...
Connection.prototype.batchInsertOrUpdate = function (insert, sql, bindParamsArray, options, callback) {
    ...
    options.useExecuteMany = ((constants.features.executeManySupport || options.forceUseExecuteMany) && (options.useExecuteMany || options.useExecuteMany === undefined) && self.executeMany && typeof self.executeMany === 'function');
 ...
 * ```
 */
Connection.prototype.executeFile = function (file, bindParams, options, callback) {
    var self = this;

    var input = self.getExecuteArguments(file, bindParams, options, callback);
    ...
    if (file) {
        self.readFile(file, function onRead(readError, sql) {
            if (readError) {
                ...
                self.execute(sql, bindParams, options, callback);
            }
        });
    } else {
...
Connection.prototype.modifyParams = function (sql, bindParams, options) {
    ...
    if (bindParams && options && options.lobMetaInfo) {
        ...
        if (lobColumns && lobColumns.length) {
            ...
            if (!options.sqlModified) {
                sql = this.generateReturningClause(sql, lobMetaInfo, lobColumns, additionalReturningInfo);

                if (options.useExecuteMany) { //mark it so we don't update the sql again
                    options.sqlModified = true;
                }
            }
        }
    }
...
module.exports = {
    ...
    extend: function extend(connection) {
        if (connection && (!connection.simplified)) {
            ...
            //remove extending capabilities if base capabilities are missing
            if (!connection.baseExecuteMany) {
                delete connection.executeMany;
            }
